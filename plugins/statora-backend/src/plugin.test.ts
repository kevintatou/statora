...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [statoraPlugin],
    });

    await request(server).get('/api/statora/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/statora/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/statora/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/statora/todos/${createdTodoItem.id}`)
